...
export async function POST(
  ...
  try {
    ...
    // Update status to FAILED with rejection message
    const updatedBot = await db.prisma.restaurantBot.update({
      where: { id: params.id },
      data: { 
        status: "FAILED",
        errorMessage: "Application rejected by administrator"
      }
    })
    ...
    try {
      ...
      const response = await fetch(`${botApiUrl}/admin/restaurants/${params.id}/reject`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      })
      
      if (!response.ok) {
        console.warn(`Failed to sync rejection to external service: ${response.statusText}`)
      }
    } catch (error) {
      console.warn("Failed to sync rejection to external service:", error)
    ...
    console.error("Reject restaurant API error:", error)
