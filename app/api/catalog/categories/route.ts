 ...
 * Fetches catalog categories with item counts
 * 
 * Headers:
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`   Categories returned: ${data.data?.categories?.length || 0}`);
    
    // Return the data as-is from the backend
    return NextResponse.json(data);
  } catch (error) {
    console.error('❌ Error in /api/catalog/categories:', error);
    return NextResponse.json(
